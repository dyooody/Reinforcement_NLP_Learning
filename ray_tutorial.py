...
initial_results, result_ids = ray.wait(result_ids, num_returns=3, timeout=None)
...
remaining_results, result_ids = ray.wait(result_ids, num_returns=3, timeout=None)
remaining_results = ray.get(remaining_results)
#print(len(remaining_results))
...
xxx = ray.put(neural_net_weights)
results = ray.get([use_weights.remote(xxx, i)
                   for i in range(20)])
